  ...
  return {
    ...(excludedLabels.length > 0
      ? {
          excludedLabels,
          labelsFor: (id: string) =>
            frontmatter.get(id.toLowerCase())?.frontmatter.labels ?? [],
        }
      : {}),
    flatDependenciesFor: (id: string) =>
      overrides[id.toLowerCase()] ??
      frontmatter.get(id.toLowerCase())?.frontmatter.dependencies ??
      [],
  };
}
...
function resolveTicketInState(
  ...
  const key = core.ticket.normalizeTicketIdForComparison(query);
  if (state.duplicateTicketIds.some((entry) => entry.ticketId === key)) {
    throw new core.CanKanError(
      core.claims.ClaimErrorCodes.TICKET_AMBIGUOUS,
      `ticket "${query}" is ambiguous`,
      { details: { ticket: query } },
    );
  }
  const matches = state.tickets.filter(
    (ticket) =>
      core.ticket.normalizeTicketIdForComparison(ticket.id) === key ||
      (ticket.displayId !== undefined &&
        core.ticket.normalizeTicketIdForComparison(ticket.displayId) === key),
  );
  if (matches.length > 1)
    throw new core.CanKanError(
      core.claims.ClaimErrorCodes.TICKET_AMBIGUOUS,
      `ticket "${query}" is ambiguous`,
      { details: { ticket: query, matches: matches.length } },
    );
...
export async function activeActors(
  ...
  const cutoff = Date.now() - core.claims.parseDurationMs(active);
