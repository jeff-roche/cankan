 ...
 * takes.
 *
 * ## A repo board's `name` is looked up in the registry first
 ...
 * directory. The exceptions are all path *comparisons*, never reads of the
 * personal board's config or tickets: comparing the canonical path of a
 * walked-up `.cankan/` root against the personal board's own canonical
 * path (`walkForBoard`), and comparing a resolved `BoardRef`'s `root`/
 * `ticketsDir` against it (`aliasesPersonalBoard`).
 *
 * ## The personal board's own `.cankan/` cannot be mistaken for a repo
 ...
 *
 * Fixed by comparing the walk's found root, canonical, against the
 * personal board's own canonical path (`canonicalPersonalPath`) --
 * comparing raw paths would be wrong on any host where `$TMPDIR` (or
 * `$HOME`) itself sits behind a symlink, which is real on macOS and is
 * exactly what this whole module's canonicalization ruling exists to get
 * right everywhere. When they match, this module resolves the personal
 * board (`kind: "personal"`, name `"personal"`, and -- since resolving to
 * the personal board is *always* "first use" per the ruling below -- via
 * `ensurePersonalBoard()`) instead of a repo board.
 *
 * The same substitution applies to an explicit `--board repo` request, not
 * only the no-flag case. A `cwd` inside the personal board's own tree is
 * not "inside an inited repo" in any sense CONCEPT.md's scope-resolution
 * rule is describing, so `--board repo` from there is treated exactly like
 * `--board repo` from outside any repo at all: a visible
 * `NOT_INSIDE_REPO_BOARD` error, never a silent substitution of one board
 * for another under an explicit selector whose whole point is to name a
 * specific board and fail loudly if it cannot.
 *
 ...
 * perspective -- there is no other place in this module that creates it
 * first. This is also what makes the returned `root` a real,
 * canonicalizable path per the ruling above: `realpath` cannot succeed
 ...
 * canonicalizes at write time, but a hand-edited `repos.yml` entry's
 * stored `path` can still be a symlink alias to the same directory `root`
 * already is. An exact-string comparison would miss that case and
 * reintroduce the "two names for one board" problem this function exists
 * to close. Every candidate has already passed `listRegisteredBoards`'s
 * own `stat` check, so `realpath` on it cannot fail with ENOENT here.
 *
 * Catches only `REGISTRY_INVALID` from `listRegisteredBoards` and falls
 * back to `undefined` rather than letting it propagate: every no-flag and
 * `--board repo` resolution would otherwise depend on `repos.yml` parsing
 * cleanly, so a single malformed byte in the user's global registry would
 * break `cd repo && cankan status`, a command with no other reason to
 * touch the registry. `--board all` is unaffected -- it calls
 * `listRegisteredBoards` directly (not through this function) and still
 * throws on the same malformed file, so the loud path survives; only a
 * single-repo session's *display name* degrades to `basename(root)`.
 * `REGISTRY_INVALID` is, as of this writing, the only error
 ...
 * function raises anything either -- but the `catch` stays narrow
 * regardless, so an error that function grows later still propagates
 * rather than silently degrading a display name.
 */
...
/**
 * True when a candidate repo board `ref` actually reaches into the
 * personal board -- checked by **containment**, on **three** directions,
 * never by equality on `root` alone:
 *
 ...
 *   `root` -- so only a check against the personal board specifically, run
 *   *after* `buildBoardRef` has resolved `ticketsDir`, can catch it.
 * - `isContained(ref.ticketsDir, personalPath)` -- the mirror shape:
 *   `tickets_dir` steered so `ticketsDir` instead *encloses* the personal
 *   board (`register($XDG_DATA_HOME, { tickets_dir: "cankan" })`, or
 *   `register($HOME, { tickets_dir: ".local" })`). This is the same
 *   dotfiles-repo-at-an-ancestor workflow the root-direction exception
 *   below protects, reached from the other side: a board's own tickets
 *   directory, not its root, containing real personal tickets and
 *   `repos.yml` itself. Not a relaxation of that exception -- this clause
 *   constrains `ticketsDir`, never `root` -- and the legitimate
 *   `$HOME`-with-default-`tickets_dir` case still passes all three checks
 *   and resolves normally (see the tests for both).
 *
 ...
 * `$XDG_DATA_HOME/cankan/personal/` on many systems. `ticketsDir` is the
 * precise cut for the enclosing shape; `root` alone cannot be.
 */
 ...
 * (or on anything else -- a symlink cycle, `ELOOP`, or an unreadable
 * ancestor, `EACCES`, must not reach a caller as a raw platform error
 * either, the same taxonomy discipline `TICKETS_DIR_INVALID` and the
 ...
 * board's own canonical path -- a path comparison only, never a read of
 * the personal board's config or tickets, so this does not violate the
 * privacy default even though it runs on every no-flag and `--board repo`
 * resolution. **By containment, not equality**: a nested `.cankan/`
 * somewhere *inside* the personal tree (`mkdir -p <personal>/proj/.cankan`,
 * then `cd` there) is exactly as much "the personal board" as its own root
 * is -- an exact-`===` check only catches a `cwd` at the personal board's
 * literal root and lets this shape through as an ordinary repo board,
 * basename name and all. A match reports `"personal-tree"` instead of
 * `"repo"` so both callers below can treat "inside the personal board's
 * own directory" as "not a repo," each according to its own precedence
 * rule.
 *
 * This closes only the *root* half of the alias check, for every cwd-based
 * resolution -- it does not (and cannot) see the *`tickets_dir`* half: a
 * `.cankan/` found here whose own config steers `tickets_dir` into the
 ...
 * `aliasesPersonalBoard`. (In practice the root-level check here ends up
 * redundant with that later, fuller check for every shape this module's
 * tests construct, since `buildBoardRef` itself enforces
 * `isContained(root, ticketsDir)` -- so `root` being inside the personal
 * board already implies `ticketsDir` is too. Kept anyway: it classifies
 * "personal-tree" directly and cheaply, without the wasted
 * `buildBoardRef`/`loadConfig` call a doomed resolution would otherwise
 * make, and it is what gives `--board repo` a clean rejection without
 * ever having built a ref for something that was never going to qualify.)
 */
 ...
 * for the `BoardFlag` union, the privacy default, and the personal-tree
 * fix that every branch below routes through.
 */
export async function resolveBoard(options: ResolveBoardOptions): Promise<BoardRef> {
  ...
  if (flag?.kind === "name") {
    ...
    if (personalPath !== undefined && aliasesPersonalBoard(ref, personalPath)) {
      throw new CanKanError(
        ...
        // No path in details -- the path in question is the personal
  ...
  if (flag?.kind === "repo") {
    if (walked?.kind !== "repo") {
      // A cwd inside the personal board's own tree is treated the same as
      // "no repo found at all" for an explicit `--board repo` request --
      // never a silent substitution of the personal board for a repo
      // board.
      throw new CanKanError(
    ...
    // `walked.kind === "repo"` only ruled out the *root* half of the alias
    // check (walkForBoard's own containment test, above) -- this repo's
    // own `tickets_dir` can still be steered into the personal board when
    // `walked.root` is an ancestor of it. Same disposition as "not inside
    // a repo board at all": an explicit `--board repo` request must fail
    // loudly, never silently substitute the personal board.
    const personalPath = await canonicalPersonalPath(env);
    if (personalPath !== undefined && aliasesPersonalBoard(ref, personalPath)) {
      throw new CanKanError(
        BoardErrorCodes.NOT_INSIDE_REPO_BOARD,
  ...
  if (walked?.kind === "repo") {
    const name = (await registeredNameFor(walked.root, env)) ?? basename(walked.root);
    const ref = await buildBoardRef({ kind: "repo", name, root: walked.root, env });
    // Same `tickets_dir`-ancestor shape as the `--board repo` branch
 ...
 * (`aliasesPersonalBoard`) before the plain canonical-root dedupe runs --
 * a registered subdirectory of the personal board, a symlink alias to it,
 * or a registered ancestor with `tickets_dir` steered into it would not
 * collide with the personal board's own *exact* root the way a plain
 * `seenRoots` check alone would need. **Deduped by canonical root**
...
export async function resolveAllBoards(options: ResolveAllBoardsOptions = {}): Promise<AllBoardsResult> {
  ...
  for (const entry of registered) {
    ...
    // Checked by containment on both `root` and `ticketsDir`
    // (`aliasesPersonalBoard`), not by `ref.root === personal.board.root`
    // -- a registered *subdirectory* of the personal board, a symlink
    // alias to it, or a registered *ancestor* with `tickets_dir` steered
    // into it all reach the personal board without `ref.root` ever
    // equaling it exactly. Checked before the dedupe check below, since
    // none of those shapes would otherwise collide with `seenRoots`
    // (which is seeded only with the personal board's own exact root).
