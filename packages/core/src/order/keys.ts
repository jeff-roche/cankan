...
export type SortKey = "id" | "title" | "status" | "priority" | "ordinal" | "rank" | "due" | "due_date" | "created" | "created_date" | "updated" | "updated_date" | "backer" | "milestone" | "age" | "random" | `label:${string}`;
export interface OrderTerm { readonly key: SortKey; readonly direction: OrderDirection; }
export interface OrderableTicket { readonly id: string; readonly title?: string; readonly status?: string; readonly priority?: string; readonly backer?: string; readonly milestone?: string; readonly labels?: readonly string[]; readonly ordinal?: number; readonly due_date?: string; readonly created_date?: string; readonly updated_date?: string; }

const PRIORITY_RANK: Readonly<Record<string, number>> = { lowest: 0, low: 1, medium: 2, normal: 2, high: 3, highest: 4, critical: 5 };
const SORT_KEYS = new Set<SortKey>(["id", "title", "status", "priority", "ordinal", "rank", "due", "due_date", "created", "created_date", "updated", "updated_date", "backer", "milestone", "age", "random"]);
...
export function parseOrder(value: string): readonly OrderTerm[] {
  ...
  return value.split(",").map((part) => {
    ...
    const rawKey = pieces[0];
    let key: SortKey;
    let rawDirection = "asc";
    if (rawKey === "label") {
      if (pieces.length < 2 || pieces[1] === "" || pieces.length > 3) throw new Error(`invalid order term: ${part.trim()}`);
      key = `label:${pieces[1]}`;
      rawDirection = pieces[2] ?? "asc";
    } else {
      if (pieces.length > 2) throw new Error(`invalid order term: ${part.trim()}`);
      key = rawKey as SortKey;
      rawDirection = pieces[1] ?? "asc";
    }
    const direction = rawDirection as OrderDirection;
    if ((!SORT_KEYS.has(key) && !key.startsWith("label:")) || (direction !== "asc" && direction !== "desc")) throw new Error(`invalid order term: ${part.trim()}`);
...
function valueFor(ticket: OrderableTicket, key: SortKey): string | number | undefined {
  switch (key) {
    ...
    case "backer": return ticket.backer ?? "";
    case "milestone": return ticket.milestone ?? "";
    case "age": case "created": case "created_date": return ticket.created_date ?? "";
    ...
    default: return ticket.labels?.includes(key.slice("label:".length)) ? 0 : 1;
  }
}
